...
export function reconstructSession(
  ...
  if (descriptor.spendLimit <= 0n) throw new Error("Altana session token spend cap must be positive");
  if (descriptor.nativeSpendLimit !== undefined && descriptor.nativeSpendLimit <= 0n) throw new Error("Altana session native spend cap must be positive when supplied");
  ...
  const spend = [
    ...(descriptor.spendToken
      ? [{ limit: descriptor.spendLimit, period: "day" as const, token: descriptor.spendToken }]
      : []),
    ...(descriptor.nativeSpendLimit !== undefined
      ? [{ limit: descriptor.nativeSpendLimit, period: "day" as const }]
      : []),
  ];

  if (spend.length === 0) throw new Error("Altana session has no spend permissions");
