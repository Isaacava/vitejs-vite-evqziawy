...
type ConsoleTab = "overview" | "execution" | "evidence" | "payments" | "timeline";
  ...
  return (
    <main className="mx-auto max-w-[1240px] px-6 py-8 md:px-8 font-body text-ink">
      ...
      <section className="card-asym-lg bg-paperhi p-6 md:p-8">
        ...
        <div className="mb-6 rounded-[16px_8px_18px_9px] border border-line bg-paper p-2 overflow-x-auto" role="tablist" aria-label="Mission console sections">
          <div className="flex min-w-max gap-1">
            {tabs.map((tab) => <button key={tab.id} type="button" role="tab" aria-selected={activeTab === tab.id} onClick={() => setActiveTab(tab.id)} className={`rounded-[12px_7px_13px_8px] px-4 py-2.5 font-mono text-[9.5px] uppercase tracking-wide transition ${activeTab === tab.id ? "bg-ink text-paperhi" : "text-inksoft hover:bg-paperhi"}`}>{tab.label}</button>)}
          </div>
        </div>
        ...
        {activeTab === "execution" && <section role="tabpanel" aria-label="Execution">
          <span className="mb-3 inline-flex items-center gap-2 font-mono text-[9.5px] uppercase tracking-widest text-brass"><span className="h-1.5 w-1.5 rounded-full bg-brass" />Execution capital · scoped session</span>
          <p className="mb-5 max-w-[650px] text-[13px] text-inksoft">Execution capital is shown only from the live request and provider-declared execution evidence. The authorization and asset state stay separate from the mission escrow.</p>
          ...
          <div className="mt-6 rounded-[16px_8px_18px_9px] border border-line bg-paper p-4">
            <div className="mb-3 flex items-center justify-between gap-3"><strong className="text-[13px] font-bold">Live execution · Testnet</strong><span className={`rounded-lg px-2.5 py-1 font-mono text-[9.5px] ${data.execution_capital ? "status-green" : "status-brass"}`}>{data.execution_capital ? "AUTHORIZED" : "PENDING"}</span></div>
        ...
        {activeTab === "evidence" && <section role="tabpanel" aria-label="Evidence">
          <span className="mb-3 inline-flex items-center gap-2 font-mono text-[9.5px] uppercase tracking-widest text-brass"><span className="h-1.5 w-1.5 rounded-full bg-brass" />Evidence</span>
          <div className="mb-6 grid gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-[16px_8px_18px_9px] border border-line bg-paper p-4"><small className="mb-1 block font-mono text-[8.5px] uppercase text-[#8a8477]">Deliverable commitment</small><strong className="font-mono text-[12px]">{compact(result?.onchain_deliverable_hash)}</strong></div>
          </div>
          <div className="rounded-[16px_8px_18px_9px] border border-line bg-paper p-4">
            ...
            {content && <pre className="mt-4 max-h-72 overflow-auto whitespace-pre-wrap break-words rounded-lg border border-linesoft bg-paperhi p-3 font-mono text-[9.5px] text-inksoft">{content}</pre>}
          </div>
        </section>}

        {activeTab === "payments" && <section role="tabpanel" aria-label="Payments">
          <span className="mb-3 inline-flex items-center gap-2 font-mono text-[9.5px] uppercase tracking-widest text-brass"><span className="h-1.5 w-1.5 rounded-full bg-brass" />Payments</span>
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-[16px_8px_18px_9px] border border-line bg-paper p-4"><small className="mb-1 block font-mono text-[8.5px] uppercase text-[#8a8477]">Quoted / funded amount</small><strong className="font-mono text-[14px]">{observed(budget)} {tokenSymbol}</strong><p className="mt-1 text-[10.5px] text-inksoft">Read from the live chain job when available.</p></div>
            <div className="rounded-[16px_8px_18px_9px] border border-line bg-paper p-4"><small className="mb-1 block font-mono text-[8.5px] uppercase text-[#8a8477]">Escrow status</small><strong className="font-mono text-[14px]">{human(liveStatus)}</strong><p className="mt-1 text-[10.5px] text-inksoft">ERC-8183 chain state is authoritative.</p></div>
            <div className="rounded-[16px_8px_18px_9px] border border-line bg-paper p-4 sm:col-span-2"><small className="mb-1 block font-mono text-[8.5px] uppercase text-[#8a8477]">Payment receipt</small><strong className="font-mono text-[12px]">{data.payment?.tx_hash ? `Recorded · ${compact(data.payment.tx_hash)}` : "No payment transaction recorded"}</strong>{data.payment?.tx_hash && <a className="mt-2 inline-block text-[10px] font-bold text-ink underline" href={bscExplorerUrl(data.payment.tx_hash as Hex)} target="_blank" rel="noreferrer">View receipt ↗</a>}</div>
          </div>
          <div className="mt-5 rounded-[16px_8px_18px_9px] border border-line bg-paper p-4"><small className="mb-2 block font-mono text-[8.5px] uppercase text-[#8a8477]">Advanced protocol detail</small><div className="grid gap-3 sm:grid-cols-2 text-[10.5px] text-inksoft"><div>Network · {data.network}</div><div>Chain ID · {data.chain_id}</div><div>Source of truth · {data.source_of_truth}</div><div>Token · {data.chain?.token_address || "Not yet observed"}</div></div></div>
        </section>}

        {activeTab === "timeline" && <section role="tabpanel" aria-label="Timeline">
          <span className="mb-3 inline-flex items-center gap-2 font-mono text-[9.5px] uppercase tracking-widest text-brass"><span className="h-1.5 w-1.5 rounded-full bg-brass" />Timeline</span>
          <p className="mb-5 max-w-[680px] text-[13px] text-inksoft">The timeline reflects the live mission record and chain status. It does not manufacture timestamps or simulate missing events.</p>
          <div className="rounded-[16px_8px_18px_9px] border border-line bg-paper overflow-hidden">
            <div className="flex items-center gap-3 p-4 dash-b"><span className="h-2 w-2 rounded-full bg-green" /><div className="flex-1"><strong className="block text-[12px] font-bold">Current chain state</strong><span className="text-[10.5px] text-inksoft">{human(liveStatus)} · authoritative</span></div><span className="font-mono text-[9px] text-[#8a8477]">Live RPC</span></div>
            {data.chain?.chain_submitted_at && <div className="flex items-center gap-3 p-4 dash-b"><span className="h-2 w-2 rounded-full bg-brass" /><div className="flex-1"><strong className="block text-[12px] font-bold">Submitted</strong><span className="text-[10.5px] text-inksoft">Chain submission timestamp observed.</span></div><span className="font-mono text-[9px] text-[#8a8477]">{new Date(data.chain.chain_submitted_at).toLocaleString()}</span></div>}
            {txHash && <div className="flex items-center gap-3 p-4"><span className="h-2 w-2 rounded-full bg-brass" /><div className="flex-1"><strong className="block text-[12px] font-bold">Latest wallet action</strong><span className="text-[10.5px] text-inksoft">Confirmed on BSC Testnet · {compact(txHash)}</span></div><a className="font-mono text-[9px] text-ink underline" href={bscExplorerUrl(txHash as Hex)} target="_blank" rel="noreferrer">Receipt ↗</a></div>}
          </div>
        </section>}
      </section>
    </main>
