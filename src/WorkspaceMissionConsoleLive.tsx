...
function policyVerdictLabel(verdict: bigint | null) {
  if (verdict === null) return "pending";
  if (verdict === 1n) return "approve";
  if (verdict === 2n) return "reject";
  return `verdict_${verdict.toString()}`;
}
    ...
    try {
      ...
      if (chainJobId && submitted) {
        try {
          ...
          if (resultResponse.ok) setResult(resultBody as JobResult);
          else setResult(null);
        } catch {
          setResult(null);
        }
        ...
        try {
          ...
          try {
            verdict = await readPolicyVerdict(BigInt(chainJobId));
          } catch {
            verdict = null;
          }
      ...
      {!data ? (
        ...
        <section className="card-asym-lg bg-paperhi p-6 md:p-8">
          <div className="grid sm:grid-cols-2 gap-4 mb-6 pb-6 dash-b">
            <div className="min-w-0"><small className="block font-mono text-[8.5px] uppercase text-[#8a8477] mb-1">Mission</small><strong className="block text-[15px] font-bold break-words">{data.mission?.title || "Agent mission"}</strong></div>
            <div className="min-w-0"><small className="block font-mono text-[8.5px] uppercase text-[#8a8477] mb-1">Agent</small><strong className="block text-[15px] font-bold break-words">{data.task?.role || "Provider"}</strong></div>
