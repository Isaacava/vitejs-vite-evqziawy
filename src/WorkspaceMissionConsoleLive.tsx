    ...
    <main className="mx-auto max-w-[1240px] px-6 py-8 md:px-8 font-body text-ink">
      ...
      <section className="card-asym-lg bg-paperhi p-6 md:p-8">
        ...
        <div className="rounded-[16px_8px_18px_9px] border border-line bg-paper p-4">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-3"><div><small className="mb-1 block font-mono text-[8px] uppercase text-[#8a8477]">Dispute window</small><strong className="font-mono text-[11px]">{remaining === null ? "Waiting for submitted timestamp" : disputeOpen ? formatTime(remaining) : "Closed"}</strong></div><div><small className="mb-1 block font-mono text-[8px] uppercase text-[#8a8477]">Settlement</small><strong className="font-mono text-[11px]">{settlementReady ? "Pending" : "Pending"}</strong></div><div><small className="mb-1 block font-mono text-[8px] uppercase text-[#8a8477]">Evaluator</small><strong className="break-all font-mono text-[11px]">{evaluator}</strong></div></div>
          <div className="mt-4 flex flex-wrap gap-2">{disputeOpen && <button className="btn-asym border border-rust px-4 py-2.5 font-display text-[11px] font-bold text-rust" disabled={!!workingAction} onClick={() => void runPolicyAction("dispute")}>Open dispute</button>}{settlementReady && <button className="btn-asym bg-ink px-4 py-2.5 font-display text-[11px] font-bold text-paperhi" disabled={!!workingAction} onClick={() => void runPolicyAction("settle")}>{workingAction === "settle" ? "Settling…" : "Settle job →"}</button>}{refundReady && <button className="btn-asym border border-brass px-4 py-2.5 font-display text-[11px] font-bold text-brass" disabled={!!workingAction} onClick={() => void runPolicyAction("refund")}>{workingAction === "refund" ? "Claiming…" : "Claim refund →"}</button>}</div>
        </div>
      </section>
    </main>
  );
