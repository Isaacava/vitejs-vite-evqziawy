...
export default function ExecutionCapitalPanel({ request, jobBudget, jobCurrency }: Props) {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        ...
        setError("");
        setLoading(false);
        if (!body?.erc8183?.submitted && body?.erc8183?.funded) timer = window.setTimeout(() => void refresh(), 3000);
      } catch (cause) {
        if (!active) return;
  ...
  return (
    <div className="mb-6 space-y-4 execution-capital-panel-universal" data-execution-mode={state?.execution_mode || "unknown"} data-hide-execution-footer={hideLegacyFooter ? "true" : "false"}>
      ...
      {!observationOnly && state?.erc8183?.submitted && request == null && <div className="text-[10px] text-inksoft">The ERC-8183 job is already submitted. Execution authorization remains optional provider evidence and is not shown as pending.</div>}
    </div>
  );
}
