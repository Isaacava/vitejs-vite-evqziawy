...
export function setPrivateNoStore(res: VercelResponse) {
  res.setHeader("Cache-Control", "private, no-store, max-age=0, must-revalidate");
  res.setHeader("Vary", "Cookie, X-AgentMarket-Wallet");
}
...
export async function getAuthenticatedUser(req: VercelRequest) {
  ...
  const requestWalletHeader = req.headers["x-agentmarket-wallet"];
  if (requestWalletHeader !== undefined) {
    const raw = Array.isArray(requestWalletHeader) ? requestWalletHeader[0] : requestWalletHeader;
    try {
      const requestWallet = getAddress(String(raw));
      if (requestWallet.toLowerCase() !== String(session.wallet_address).toLowerCase()) return null;
    } catch {
      return null;
    }
  }
  ...
  if (!user || String(user.wallet_address).toLowerCase() !== String(session.wallet_address).toLowerCase()) return null;
  return { user, session };
}
...
export async function verify(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!session || session.revoked_at || session.verified_at || new Date(session.expires_at).getTime() <= Date.now()) return res.status(401).json({ error: "Sign-in challenge is invalid, already used, or expired" });
