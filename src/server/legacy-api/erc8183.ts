...
async function syncReceipt(req: VercelRequest, res: VercelResponse, auth: AuthenticatedUser) {
  ...
  await supabase.from("user_activity").insert({ user_id: auth.user.id, mission_id: mission.id, job_id: job.id, type: "chain_receipt_confirmed", title: `${phase} transaction confirmed`, description: `Verified on BSC Mainnet: ${txHash}`, metadata: { phase, tx_hash: txHash, chain_job_id: chainJobId || null, block_number: receipt.blockNumber.toString(), receipt_status: receipt.status, chain_id: 56 } });
  return res.status(200).json({ ok: true, phase, network: "bsc-mainnet", chain_id: 56, tx_hash: txHash, block_number: receipt.blockNumber.toString(), receipt_status: receipt.status, job: updatedJob, onchain_job: confirmedJob ? { id: confirmedJob.id.toString(), status: Number(confirmedJob.status), budget: confirmedJob.budget.toString(), provider: confirmedJob.provider, client: confirmedJob.client } : null, note: "Database state advanced only after a successful BSC Mainnet receipt and matching production ERC-8183 contract target." });
}
