...
async function resolveGridProvider(): Promise<Address> {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 8_000);
  try {
    const response = await fetch(`${GRID_AGENT_URL}/erc8183/status`, {
      headers: { Accept: "application/json" },
      signal: controller.signal,
    });
    if (!response.ok) throw new Error(`Grid Agent status returned ${response.status}`);
    const body = await response.json() as { network?: string; chain_id?: number; agent_address?: string };
    if (body.network !== "bsc-testnet" || Number(body.chain_id) !== 97) throw new Error("Grid Agent runtime is not reporting BSC Testnet");
    if (typeof body.agent_address !== "string" || !/^0x[a-fA-F0-9]{40}$/.test(body.agent_address)) throw new Error("Grid Agent runtime did not expose a valid provider wallet");
    return body.agent_address as Address;
  } finally {
...
async function syncReceipt(req: VercelRequest, res: VercelResponse, auth: NonNullable<Awaited<ReturnType<typeof getAuthenticatedUser>>>) {
  ...
  if (phase === "create" && !chainJobId) {
    ...
    const runtimeProvider = await resolveGridProvider();
    if (args.provider && args.provider.toLowerCase() !== runtimeProvider.toLowerCase()) return res.status(409).json({ error: "JobCreated provider does not match the live Testnet Grid Agent runtime", expected_provider: runtimeProvider, actual_provider: args.provider });
  ...
  if (chainJobId) {
    ...
    const runtimeProvider = await resolveGridProvider();
    if (confirmedJob.provider.toLowerCase() !== runtimeProvider.toLowerCase()) return res.status(409).json({ error: "Chain job provider does not match the live Testnet Grid Agent runtime", expected_provider: runtimeProvider, actual_provider: confirmedJob.provider });
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { data: agent, error: agentError } = await supabase.from("agents").select("agent_id,name,verification_status,status,metadata").eq("id", task.agent_id).maybeSingle();
    if (agentError) throw new Error(agentError.message);
    if (!agent || agent.agent_id !== "grid-strategy" || agent.metadata?.environment !== "testnet" || agent.metadata?.network !== "bsc-testnet") return res.status(409).json({ error: "Selected provider is not the isolated Testnet Grid Agent" });
    const provider = await resolveGridProvider();
    if (agent.status !== "online" || agent.verification_status === "revoked") return res.status(409).json({ error: "Testnet Grid Agent is not currently available" });
    ...
    const createJobData = encodeFunctionData({ abi: COMMERCE_ABI, functionName: "createJob", args: [provider, ROUTER, expiresAt, mission.goal, ROUTER] });
    ...
    return res.status(200).json({ ok: true, network: "bsc-testnet", chain_id: 97, mission: { id: mission.id, status: mission.status }, agent: { agent_id: agent.agent_id, name: agent.name, provider, status: agent.status, verification_status: agent.verification_status }, commerce: { address: COMMERCE, evaluator: ROUTER, hook: ROUTER, default_policy: POLICY }, payment: { token, symbol, decimals, budget_raw: rawBudget.toString(), balance_raw: String(balance), allowance_raw: String(allowance), balance_formatted: formatUnits(BigInt(balance), Number(decimals)), allowance_formatted: formatUnits(BigInt(allowance), Number(decimals)) }, expiry: new Date(Number(expiresAt) * 1000).toISOString(), wallet_steps: ["createJob", "registerJob using the returned jobId", "setBudget using the returned jobId", BigInt(allowance) < rawBudget ? "approve payment token" : "approval already sufficient", "fund using the returned jobId"], transactions: { create_job: { to: COMMERCE, data: createJobData }, register_job: { to: ROUTER, data: registerTemplate, data_builder: "Replace placeholder jobId 0 with the confirmed createJob receipt jobId." }, set_budget: { to: COMMERCE, data: setBudgetTemplate, data_builder: "Replace placeholder jobId 0 with the confirmed createJob receipt jobId." }, approve: BigInt(allowance) < rawBudget ? { to: token, data_builder: "Encode ERC-20 approval for Commerce and the requested Testnet budget." } : { data_builder: "No approval transaction required." }, fund: { to: COMMERCE, data: fundTemplate, data_builder: "Replace placeholder jobId 0 with the confirmed createJob receipt jobId." } }, note: "Testnet-only preparation for isolated Grid Agent validation. The provider wallet is resolved from the live Testnet Railway runtime, never from a Mainnet agent record." });
