...
type ExecutionSummary = {
  ...
  execution?: { status?: string; block_number?: string } | null;
  market?: { verified_onchain?: boolean } | null;
};
...
function Status({ value }: { value: string }) {
  ...
  return <span className={`font-mono text-[9.5px] px-2.5 py-1 rounded-lg ${tone}`}>{human(state)}</span>;
}
...
function Lifecycle({ state }: { state: string }) {
  ...
  return <div className="grid grid-cols-4 overflow-hidden rounded-[12px_7px_13px_8px] bg-deep">
    {["open", "executing", "review", "settled"].map((label, index) => <div key={label} className="border-r border-white/10 p-2.5 last:border-r-0">
...
export default function DemoOverviewPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!jobsResponse.ok) throw new Error(history.error || "Unable to load Testnet history");
        if (history.network !== "bsc-testnet" || Number(history.chain_id) !== 97) throw new Error("Testnet history returned an unexpected network.");
        if (!active) return;
        setData(dashboard);
        setJobs(Array.isArray(history.jobs) ? history.jobs : []);
        const primary = (history.jobs || []).find((job) => activeStates.includes(chainState(job)));
        if (primary?.id) {
          const evidenceResponse = await fetch(`/api/testnet?route=execution-evidence&job=${encodeURIComponent(primary.id)}`, { credentials: "include", cache: "no-store" });
          if (evidenceResponse.ok && active) setExecution(await evidenceResponse.json() as ExecutionSummary);
        }
      } catch (cause) {
  ...
  const capitalAuthorized = Boolean(execution?.observed && execution?.market?.verified_onchain);
  const capitalNote = capitalAuthorized ? "active on-chain execution evidence" : "no active session returned";
  ...
  return <main className="mx-auto max-w-[1240px] px-6 py-8 md:px-8 text-ink">
    ...
    <div className="relative overflow-hidden">
      ...
      <section className="relative z-10 mb-8 grid gap-6 lg:grid-cols-[1.5fr_.65fr]">
        <div>
          <span className="inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-brass mb-3"><span className="h-1.5 w-1.5 rounded-full bg-brass" /> User operating system / 01</span>
          ...
          <p className="mt-3 max-w-[500px] text-[13.5px] leading-relaxed text-inksoft">Active work, chain-verified progress, evaluation state, payment tracking and the evidence behind each mission.</p>
        </div>
        <div className="card-asym rotate-[1deg] overflow-hidden border border-[#b9b09a] bg-paperhi shadow-[0_30px_60px_-36px_rgba(23,23,20,.35)]">
          <div className="flex justify-between border-b border-dashed border-[#c8c0af] px-4 py-2.5 font-mono text-[8.5px] text-[#8a8477]"><span>CONNECTED WALLET</span><span className="text-green">● SIGNED IN</span></div>
          <div className="px-4 py-4"><small className="mb-1.5 block font-mono text-[9px] uppercase text-[#8a8477]">Address</small><strong className="block text-[16px] font-bold">{compact(data?.user.wallet_address)}</strong></div>
          <div className="flex items-center justify-between border-t border-dashed border-line bg-[#f2f0e7] px-4 py-3"><span className="text-[10.5px] text-inksoft">Escrow authority scoped</span><a href="/app" className="text-[11.5px] font-extrabold no-underline">New mission <span className="text-brass">→</span></a></div>
        </div>
      </section>
      ...
      {loading ? <div className="py-16 text-[13px] text-inksoft">Loading your mission state…</div> : <>
        <div className="relative z-10 mb-6 grid grid-cols-2 gap-3.5 md:grid-cols-5">
          <Metric label="Active missions" value={activeJobs.length} note="funded · accepted · executing" />
          ...
          <Metric label="Escrow tracked" value={escrow.toLocaleString()} note="chain-backed Testnet value" brass />
          <Metric label="Capital authorized" value={capitalAuthorized ? 1 : "—"} note={capitalNote} brass />
        </div>
        ...
        <div className="relative z-10 mb-4 grid gap-4 lg:grid-cols-[1.5fr_1fr]">
          <section className="card-asym border border-line bg-paperhi p-[18px]">
            <div className="mb-1 flex items-center justify-between border-b border-dashed border-[#c8c0af] pb-3"><span className="font-mono text-[9.5px] uppercase tracking-wide text-[#8a8477]">02 / Current work</span><a href="/missions" className="text-[11px] font-extrabold text-brass underline underline-offset-4 no-underline">View all →</a></div>
            {currentWork.length ? currentWork.map((job) => {
              ...
              return <article key={job.chain_job_id} className="border-b border-linesoft py-4 last:border-b-0">
                <div className="flex flex-col gap-3.5 sm:flex-row sm:justify-between">
                  <div className="min-w-0"><div className="font-mono text-[9.5px] uppercase tracking-wide text-[#8a8477]">{human(job.task_title || "Testnet")}</div><h2 className="mt-1 text-[14.5px] font-bold">{job.mission_title || job.task_title || "Mission"}</h2><p className="max-w-[340px] text-[11.5px] text-inksoft">{job.description || "Chain-backed Testnet mission."}</p></div>
                  <div className="shrink-0 sm:min-w-[140px] sm:text-right"><Status value={state}/><span className="my-1.5 block text-[11px] text-inksoft">Grid Strategy Agent</span><a href={`/mission?job=${encodeURIComponent(String(job.id || ""))}`} className="text-[11px] font-extrabold text-brass no-underline">Open →</a></div>
                </div>
                <div className="mt-3"><Lifecycle state={state}/></div>
                <div className="mt-2 font-mono text-[9px] text-[#8a8477]">synced {ago(job.updated_at)}</div>
              </article>;
            }) : <div className="py-8"><strong className="font-display text-[21px]">No active missions</strong><p className="mt-2 text-[12px] text-inksoft">Describe an outcome and hire an agent to create your first mission.</p><a href="/app" className="text-[11px] font-extrabold text-brass no-underline">Create mission →</a></div>}
          </section>

          <aside className="card-asym border border-line bg-paperhi p-[18px]">
            <div className="mb-1 flex items-center justify-between border-b border-dashed border-[#c8c0af] pb-3"><span className="font-mono text-[9.5px] uppercase tracking-wide text-[#8a8477]">03 / Activity</span><a href="/activity" className="text-[11px] font-extrabold text-brass underline underline-offset-4 no-underline">Full log →</a></div>
            {activities.slice(0, 6).map((event, index) => <div key={event.id} className={`py-3 ${index < Math.min(activities.length, 6) - 1 ? "border-b border-linesoft" : ""}`}><strong className="block text-[12px] font-bold">{event.title}</strong><p className="my-0.5 text-[10.5px] text-inksoft">{event.description || human(event.title)}</p><small className="font-mono text-[9.5px] text-[#9aa3b1]">{ago(event.created_at)}</small></div>)}
            {!activities.length && <div className="py-8 text-[12px] text-inksoft">No activity records yet.</div>}
          </aside>
        </div>

        <section className="relative z-10 card-asym border border-line bg-paperhi p-[18px]">
          <div className="mb-1 flex items-center justify-between border-b border-dashed border-[#c8c0af] pb-3"><span className="font-mono text-[9.5px] uppercase tracking-wide text-[#8a8477]">04 / Recent missions</span><a href="/missions" className="text-[11px] font-extrabold text-brass underline underline-offset-4 no-underline">Mission log →</a></div>
          {jobs.slice(0, 8).map((job) => <div key={job.chain_job_id} className="flex flex-col gap-2 border-b border-linesoft py-3.5 last:border-b-0 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><strong className="block text-[13px] font-bold">{job.mission_title || job.task_title || "Testnet mission"}</strong><span className="text-[11px] text-inksoft">{human(job.task_title || "Testnet task")}</span></div><Status value={chainState(job)}/><small className="font-mono text-[10px] text-[#9aa3b1] sm:min-w-[130px] sm:text-right">{ago(job.updated_at)}</small></div>)}
          {!jobs.length && <div className="py-8 text-[12px] text-inksoft">No chain jobs recorded yet.</div>}
        </section>
      </>}
    </div>
  </main>;
