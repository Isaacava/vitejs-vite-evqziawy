...
const stateLabel = (value: string) => {
  ...
  if (state === "completed" || state === "terminal") return "Completed";
...
function Lifecycle({ state }: { state: string }) {
  ...
  const active = terminal.includes(lower) ? 4 : lower === "submitted" ? 3 : ["funded", "accepted", "in_progress"].includes(lower) ? 1 : 0;
  return <div className="grid grid-cols-2 sm:grid-cols-5 overflow-hidden rounded-[12px_7px_13px_8px] bg-deep" aria-label={`Mission progress: ${stateLabel(lower)}`}>
    {["Open", "Funded", "Accepted", "In progress", "Submitted"].map((label, index) => <div key={label} className="border-r border-white/10 p-2.5 last:border-r-0">
    ...
    <div className="relative overflow-hidden">
      ...
      <section className="relative z-10 mb-8 grid gap-6 lg:grid-cols-[1.5fr_.65fr]">
        <div>
          ...
          <h1 className="font-display text-[32px] font-bold tracking-tight leading-[1.03] md:text-[42px]">Your missions,<br/><em className="not-italic text-brass">chain-verified.</em></h1>
          <p className="mt-3 max-w-[560px] text-[13.5px] leading-relaxed text-inksoft">Mission state shown here is read from BSC Testnet first — Supabase is the workflow layer, never the source of truth.</p>
        </div>
        <div className="card-asym rotate-[1deg] overflow-hidden border border-[#b9b09a] bg-paperhi shadow-[0_30px_60px_-36px_rgba(23,23,20,.35)]">
          ...
          <div className="flex items-center justify-between border-t border-dashed border-line bg-[#f2f0e7] px-4 py-3"><span className="text-[10.5px] text-inksoft">Escrow authority scoped</span><a href="/app" className="shrink-0 text-[11.5px] font-extrabold no-underline">New mission <span className="text-brass">→</span></a></div>
        </div>
      </section>
      ...
      {loading ? <div className="py-16 text-[13px] text-inksoft">Loading your mission state…</div> : <>
        <div className="relative z-10 mb-6 grid grid-cols-2 gap-3.5 md:grid-cols-5">
          ...
          <Metric label="Escrow tracked" value={escrow.toLocaleString()} note="tBNB, DB view" brass />
          <Metric label="Capital authorized" value="—" note="Only shown when an active scoped session is returned" brass />
        </div>
            ...
            {currentWork.length ? currentWork.map((job) => {
              ...
              return <article key={job.chain_job_id} className="border-b border-linesoft py-4 last:border-b-0">
                <div className="flex flex-col gap-3.5 sm:flex-row sm:justify-between">
                  ...
                  <div className="shrink-0 sm:min-w-[160px] sm:text-right"><Status value={state}/><span className="my-1.5 block text-[11px] text-inksoft">{job.agent?.name || "Assigned provider"}</span></div>
                </div>
                <div className="mt-3"><Lifecycle state={state}/></div>
                <div className="mt-3 flex flex-wrap items-center gap-2 font-mono text-[9px] text-[#8a8477]"><a href={`/mission?job=${encodeURIComponent(String(job.id || ""))}`} className="shrink-0 text-[11px] font-extrabold text-brass no-underline">Open →</a><span>Chain: {state}</span><span>·</span><span>{job.mission_id ? "active" : "chain job"}</span><span>·</span><span>synced {ago(job.updated_at)}</span></div>
              </article>;
            }) : <div className="py-8"><strong className="font-display text-[21px]">No active missions</strong><p className="mt-2 text-[12px] text-inksoft">Describe an outcome in plain English and AgentMarket will help you find a suitable provider.</p><a href="/app" className="text-[11px] font-extrabold text-brass no-underline">Create mission →</a></div>}
          </section>

          <aside className="card-asym border border-line bg-paperhi p-[18px]">
            <div className="mb-1 flex items-center justify-between border-b border-dashed border-[#c8c0af] pb-3"><span className="font-mono text-[9.5px] uppercase tracking-wide text-[#8a8477]">03 / Activity</span><a href="/activity" className="text-[11px] font-extrabold text-brass no-underline">Full log →</a></div>
            {activities.slice(0, 6).map((event, index) => <div key={event.id} className={`py-3 ${index < Math.min(activities.length, 6) - 1 ? "border-b border-linesoft" : ""}`}><strong className="block text-[12px] font-bold">{event.title}</strong><p className="my-0.5 text-[10.5px] text-inksoft">{event.description || "AgentMarket activity recorded."}</p><small className="font-mono text-[9.5px] text-[#9aa3b1]">{ago(event.created_at)}</small></div>)}
