...
const publicClient = createPublicClient({ chain: bscTestnet, transport: http("https://bsc-testnet-rpc.publicnode.com", { timeout: 8000, retryCount: 1 }) });
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    void getCurrentUser().then((current) => {
      ...
      const resolved = current as User;
      setUser(resolved);
      void loadFaucet(resolved.wallet_address as Address);
    });
  }, []);

  async function loadFaucet(walletAddress?: Address) {
    const wallet = walletAddress || (user?.wallet_address as Address | undefined);
    if (!wallet) return;
    setFaucetLoading(true);
    try {
      const [tBNB, U, CAKE2Balance] = await Promise.all([
        publicClient.getBalance({ address: wallet }),
        publicClient.readContract({ address: U_TOKEN, abi: ERC20_ABI, functionName: "balanceOf", args: [wallet] }),
        publicClient.readContract({ address: CAKE2, abi: ERC20_ABI, functionName: "balanceOf", args: [wallet] }),
      ]);
      const missing = {
        tBNB: tBNB < REQUIRED_TBNB ? REQUIRED_TBNB - tBNB : 0n,
        U: U < REQUIRED_U ? REQUIRED_U - U : 0n,
        CAKE2: CAKE2Balance < REQUIRED_CAKE2 ? REQUIRED_CAKE2 - CAKE2Balance : 0n,
      };
      setFaucet({
        user: { tBNB: formatEther(tBNB), U: formatUnits(U, 18), CAKE2: formatUnits(CAKE2Balance, 18) },
        required: { tBNB: "0.02", U: "2", CAKE2: "5" },
        missing: { tBNB: formatEther(missing.tBNB), U: formatUnits(missing.U, 18), CAKE2: formatUnits(missing.CAKE2, 18) },
        ready: missing.tBNB === 0n && missing.U === 0n && missing.CAKE2 === 0n,
        faucet: { available: true },
      });
      setFaucetMessage("");
    } catch (cause) {
      setFaucetMessage(cause instanceof Error ? `Balance check unavailable right now: ${cause.message}` : "Balance check unavailable right now.");
    } finally {
  ...
  async function claimFaucet() {
    ...
    try {
      const response = await fetch("/api/testnet?route=faucet", { method: "POST", credentials: "include", cache: "no-store" });
      ...
      setFaucet(body);
      if (body.ready) {
        setFaucetMessage("Testnet faucet sent 2 U, 0.02 tBNB and 5 CAKE2 to your AgentMarket wallet.");
        setStep(4);
      } else {
        setFaucetMessage("Testnet faucet sent funds. Refreshing your wallet balances…");
        await loadFaucet(user?.wallet_address as Address);
      }
