...
const REQUIRED_TBNB = parseUnits("0.002", 18);
...
export default function OnboardingPage() {
  ...
  async function claimFaucet() {
    ...
    try {
      ...
      setFaucet(body); setFaucetMessage(body.ready ? "Test funds are now in your AgentMarket wallet." : "Claim completed; checking balances again…");
      await loadFaucet();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Testnet faucet claim failed."); }
    finally { setFaucetBusy(false); }
  }

  async function refreshAltanaBalances(wallet = altana?.walletAddress) {
    if (!wallet) return;
    ...
    setAltanaBalances({ tBNB, CAKE2: CAKE2Balance });
  }
  ...
  const altanaReady = Boolean(altanaBalances && altanaBalances.tBNB >= REQUIRED_TBNB && altanaBalances.CAKE2 >= REQUIRED_CAKE2);
  ...
  async function fundAltana() {
    ...
    try {
      const current = altanaBalances || await (async () => { await refreshAltanaBalances(altana.walletAddress); return altanaBalances; })();
      if (!current || current.tBNB < REQUIRED_TBNB) {
