...
export default function AltanaSessionGrantGate(props: AltanaSessionGrantGateProps) {
  ...
  async function grant() {
    ...
    try {
      ...
      // IMPORTANT: the user grants the scoped session BEFORE any execution capital
      // funding. AgentMarket verifies the grant on-chain before the flow proceeds.
      setStatus("signing");
      ...
      // Only after on-chain verification succeeds do we fund the execution wallet
      // and create the allowance required by the agent's local execution layer.
  ...
  const symbol = props.capitalSymbol || "execution token";
  const statusText = status === "checking" ? "Checking request-scoped Testnet session and fee readiness…" : status === "signing" ? "Waiting for Passkey approval…" : status === "verifying" ? "AgentMarket is verifying the Altana grant on-chain…" : status === "funding_capital" ? `Fund ${props.capitalAmount.toString()} ${symbol} to your Altana execution wallet…` : status === "approving_allowance" ? `Approve ${props.capitalAmount.toString()} ${symbol} router allowance in your Altana wallet…` : status === "authorized" ? "Authorized and independently verified ✓" : props.renewal ? `Authorize a fresh ${symbol} session for this job` : `Approve the agent's ${symbol} trading authority`;

  return (
    <section className="border border-line rounded-[16px_8px_18px_9px] bg-paper p-5">
      <div className="flex items-start justify-between gap-4"><div><small className="block font-mono text-[8.5px] uppercase tracking-widest text-brass mb-1.5">Execution Capital · Altana Session</small><h3 className="font-display text-[18px] font-bold m-0">{props.renewal ? "Renew the job's trading authority" : "Approve the agent's trading authority"}</h3><p className="text-[11px] text-inksoft mt-1.5 max-w-[600px]">The execution token below is the exact token declared by the selected agent for this request. AgentMarket funds only that token after the Altana grant has been verified.</p></div><span className={`font-mono text-[9px] px-2.5 py-1 rounded-lg ${status === "authorized" ? "status-green" : "status-brass"}`}>BSC TESTNET</span></div>
      ...
      <div className="mt-4 border border-line rounded-[12px_7px_13px_8px] bg-paperhi p-4"><small className="block font-mono text-[8px] uppercase text-[#8a8477] mb-2">Authorization sequence</small><div className="text-[10.5px] text-inksoft leading-5">1. User signs the scoped Altana session. 2. AgentMarket verifies the session on-chain. 3. Only then is execution capital funded and the execution allowance created.</div>{capitalTxHash && <a className="inline-block mt-2 text-[9px] font-mono text-brass break-all" href={`https://testnet.bscscan.com/tx/${capitalTxHash}`} target="_blank" rel="noreferrer">Execution-capital transfer ↗</a>}</div>
