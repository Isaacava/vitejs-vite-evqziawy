...
function missingReceiptError(value?: string) { if (!value) return false; const normalized = value.toLowerCase(); return normalized.includes("transaction receipt") && (normalized.includes("could not be found") || normalized.includes("not processed on a block") || normalized.includes("receipt not found")); }
function transactionHashFromError(value?: string) { return value?.match(/0x[a-fA-F0-9]{64}/)?.[0] || null; }
...
export default function ExecutionCapitalLivePanel({ request }: Props) {
        ...
        if (!response.ok) {
          ...
          if (missingReceiptError(message)) {
            ...
            if (candidateHash) {
              ...
              } catch { /* retain candidate hash as pending evidence */ }
            }
            setPending(Boolean(candidateHash)); setError(""); return;
          }
          throw new Error(message);
        }
        setExecution(body); setPending(Boolean(body?.observed === false && transactionHashFromExecution(body))); setError("");
      } catch (cause) {
  ...
  const allowedCalls = Array.isArray(capability.allowed_targets) ? capability.allowed_targets.filter((value): value is Address => typeof value === "string" && /^0x[a-fA-F0-9]{40}$/.test(value)) : [];
  ...
  const capitalAmount = (() => { try { const value = BigInt(request.capital_requested || "1"); return value > 0n ? value : 1n; } catch { return 1n; } })();
  ...
  const needsAuthorization = !authorizationVerified && request.status !== "expired" && request.status !== "revoked";
  ...
  const jobScopedCapabilityUrl = capabilitySourceUrl ? `${capabilitySourceUrl}${capabilitySourceUrl.includes("?") ? "&" : "?"}job_id=${encodeURIComponent(request.job_id)}` : "";

  return (
    <section className="border border-line rounded-[16px_8px_18px_9px] bg-paper p-5">
      <div className="flex items-start justify-between gap-4"><div><small className="block font-mono text-[8.5px] uppercase tracking-widest text-brass mb-1">Live execution · Testnet</small><h3 className="font-display text-[18px] font-bold m-0">Agent execution</h3><p className="text-[10.5px] text-inksoft mt-1">Execution evidence is independently read from BSC Testnet. AgentMarket does not dispatch the agent or sign execution calls.</p></div><span className={`font-mono text-[9px] px-2.5 py-1 rounded-lg ${verifiedExecution ? "status-green" : pending ? "status-brass" : error ? "status-rust" : "status-brass"}`}>{verifiedExecution ? "ONCHAIN VERIFIED" : pending ? "PENDING RECEIPT" : error ? "VERIFICATION ERROR" : "NOT YET OBSERVED"}</span></div>
      <div className="mt-4 border border-line rounded-[12px_7px_13px_8px] bg-paperhi p-4"><small className="block font-mono text-[8px] uppercase tracking-widest text-brass mb-2">Execution evidence</small><div className="grid sm:grid-cols-2 gap-x-6 gap-y-2 text-[10px]"><div><strong>Execution asset:</strong> {tokenInSymbol} → {tokenOutSymbol}</div><div><strong>Actual amount:</strong> {amountIn} {tokenInSymbol} → {amountOut} {tokenOutSymbol}</div><div><strong>Token in:</strong> {market?.token_in ? compact(market.token_in) : "Not yet observed"}</div><div><strong>Token out:</strong> {market?.token_out ? compact(market.token_out) : "Not yet observed"}</div><div><strong>Pool fee:</strong> {market?.fee ?? "Not yet independently identified"}</div><div><strong>Pool:</strong> {market?.pool ? compact(market.pool) : "Not yet independently identified"}</div><div><strong>Execution wallet:</strong> {wallet ? compact(wallet) : "Not yet observed"}</div><div><strong>Receipt:</strong> {execution?.execution?.status || "Not yet observed"}</div><div><strong>Block:</strong> {execution?.execution?.block_number || "Not yet observed"}</div><div><strong>Gas used:</strong> {execution?.execution?.gas_used || "Not yet observed"}</div></div>{execution?.transaction_hash && <div className="mt-3 text-[10px]"><strong>Transaction:</strong>{" "}<a className="font-mono text-brass underline break-all" href={`https://testnet.bscscan.com/tx/${execution.transaction_hash}`} target="_blank" rel="noreferrer">{compact(execution.transaction_hash)} ↗</a></div>}</div>
      ...
      {needsAuthorization && canRenderAuthorization && <div className="mt-5"><AltanaSessionGrantGate requestId={request.id} agentSessionAddress={sessionAddress as Address} agentSessionPublicKey={sessionPublicKey as Hex} allowedCalls={allowedCalls} allowedSelectors={allowedSelectors} capitalAmount={capitalAmount} capitalToken={capitalToken} capitalSymbol={typeof capabilityMarket.token_in_symbol === "string" ? capabilityMarket.token_in_symbol : tokenInSymbol} capitalDecimals={18} approvalSpender={approvalSpender} purpose={request.purpose} durationSeconds={request.requested_duration_seconds || request.duration_seconds || 86400} capabilitySource={jobScopedCapabilityUrl} onAuthorized={() => window.location.reload()} /></div>}
    </section>
  );
}
