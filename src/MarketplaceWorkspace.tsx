...
export default function MarketplaceWorkspace() {
  ...
  function structuredParameters() {
    if (!best) return {};
    if (best.agent.category === "grid_trading") return gridParams(best.agent.category);
  ...
  function validateParameters(parameters: Record<string, unknown>) {
    ...
    if (best.agent.category === "rebalancing") {
      const lower = Number(parameters.tick_lower);
      const upper = Number(parameters.tick_upper);
      if (Number.isFinite(lower) && Number.isFinite(upper) && upper <= lower) throw new Error("Upper tick must be greater than lower tick.");
    }
    if (best.agent.category === "health_factor" && Number(parameters.health_factor) <= 0) throw new Error("Health factor must be positive.");
  }
    ...
    <main className="max-w-[1240px] mx-auto px-6 md:px-8 py-8 relative font-body text-ink">
      ...
      <div className="bg-paperhi border border-line card-asym-lg p-6 md:p-8 min-h-[420px]">
        ...
        {step === 3 && best && !quote && <>
          ...
          <p className="text-[13px] text-inksoft mb-5 max-w-[560px]">These questions come from the selected agent's capability schema. Only required inputs are requested; Grid keeps its existing defaults.</p>
