...
function Info({ title, value, mono = false, green = false }: { title: string; value: string; mono?: boolean; green?: boolean }) {
  return <div className="border border-[#d5cfbf] rounded-[12px_7px_13px_8px] bg-[#eeeade] p-3.5"><small className="block font-mono text-[8.5px] uppercase text-[#8a8477] mb-1.5">{title}</small><strong className={`${mono ? "font-mono text-[11px] leading-relaxed" : "font-[Space_Grotesk,sans-serif] text-[15px]"} ${green ? "text-[#2d6b4f]" : ""}`}>{value}</strong></div>;
}
...
export default function MarketplaceWorkspace() {
  ...
  useEffect(() => { if (new URLSearchParams(location.search).get("funded") === "1") setStep(6); }, []);
  ...
  async function findAgent() {
    ...
    try {
      const r = await fetch("/api/testnet/match", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ goal }) });
      const b = await read(r);
      const next = b as MatchResponse;
      setResult(next);
      setSelected(next.bestHireableMatch || next.bestMatch);
      return true;
    } catch (e) {
      setError(e instanceof Error ? e.message : "Unable to find a Testnet agent");
      setResult(null);
      setSelected(null);
      return false;
    } finally {
  ...
  return <main className="max-w-[1240px] mx-auto px-6 md:px-8 py-8 relative font-[Manrope,sans-serif] text-[#171714]">
    ...
    <div className="bg-[#fbfaf5] border border-[#d5cfbf] rounded-[26px_10px_28px_13px] p-6 md:p-8 min-h-[420px]">
      ...
      {step === 5 && prepared && <><span className="inline-flex items-center gap-2 font-mono text-[9.5px] uppercase tracking-widest text-[#9d7428] mb-3"><span className="w-1.5 h-1.5 rounded-full bg-[#9d7428]" />Step 5 / 6 · ERC-8183 preparation</span><h2 className="font-[Space_Grotesk,sans-serif] text-[24px] font-bold tracking-tight mb-1">Your wallet signs the job steps.</h2><p className="text-[13px] text-[#6d6a61] mb-5 max-w-[560px]">The backend has prepared the real transaction payloads. Wallet execution remains client-side and non-custodial.</p><div className="border border-[#d5cfbf] rounded-[18px_9px_20px_10px] overflow-hidden mb-6"><div className="p-4 border-b border-dashed border-[#c8c0af] bg-[#eeeade]"><Info title="Token" value={prepared.payment.symbol} mono /><Info title="Balance" value={prepared.payment.balance_formatted} mono /></div><div className="p-5 bg-[#eeeade]"><div className="text-[12px] text-[#6d6a61] mb-2">Prepared quote: <span className="font-mono">{prepared.quote.price} {prepared.quote.currency}</span></div><div className="text-[11px] text-[#6d6a61]">{prepared.job_description}</div></div></div><div className="flex gap-3"><button onClick={() => location.assign(`/testnet/execute?mission=${encodeURIComponent(mission?.mission.id || "")}`)} className="font-[Space_Grotesk,sans-serif] font-bold text-[12px] px-5 py-3 bg-[#171714] text-[#fbfaf5] rounded-[14px_8px_16px_9px]">Open wallet execution →</button><button onClick={() => setStep(4)} className="font-bold text-[12px] px-4 py-3 text-[#6d6a61]">← Back</button></div></>}

      {step === 6 && <><span className="inline-flex items-center gap-2 font-mono text-[9.5px] uppercase tracking-widest text-[#2d6b4f] mb-3"><span className="w-1.5 h-1.5 rounded-full bg-[#2d6b4f]" />Step 6 / 6 · Chain receipt</span><h2 className="font-[Space_Grotesk,sans-serif] text-[24px] font-bold tracking-tight mb-1">Funded, on-chain.</h2><p className="text-[13px] text-[#6d6a61] mb-5 max-w-[560px]">The wallet execution route has returned to the hiring flow. Continue from the real job record rather than assuming a transaction succeeded.</p><div className="border border-[#d5cfbf] rounded-[18px_9px_20px_10px] p-5 mb-6 bg-[#eeeade] grid sm:grid-cols-2 gap-4"><Info title="Status" value="Funded" green /><Info title="Network" value="BSC Testnet · Chain 97" /><Info title="Mission" value={mission?.mission.title || "Testnet mission"} /><Info title="Job" value={compact(mission?.job.id)} mono /></div><div className="flex gap-3"><button onClick={() => location.assign(`/dashboard?tab=missions`)} className="font-[Space_Grotesk,sans-serif] font-bold text-[12px] px-5 py-3 bg-[#171714] text-[#fbfaf5] rounded-[14px_8px_16px_9px]">Go to Missions →</button><button onClick={() => location.assign(`/dashboard?job=${encodeURIComponent(mission?.job.id || "")}`)} className="font-bold text-[12px] px-4 py-3 text-[#6d6a61]">Open mission console</button></div></>}
    </div>
  </main>;
}