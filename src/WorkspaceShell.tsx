    ...
    <div className="min-h-screen bg-paper text-ink antialiased font-body">
      <header className={`sticky top-0 z-50 border-b border-line bg-paper/90 backdrop-blur transition-shadow duration-200 ${scrolled ? "shadow-[0_8px_24px_rgba(23,23,20,.06)]" : ""}`}>
        <div className="mx-auto flex h-[72px] max-w-[1240px] items-center justify-between gap-6 px-6 md:px-8">
          ...
          <nav className="hidden items-center gap-7 font-mono text-[11px] font-medium uppercase tracking-wide text-inksoft lg:flex">
            {primaryLinks.map((link) => (
              <button
                ...
                className={`nav-link relative pb-1 transition-colors ${page === link.label ? "text-ink current" : "hover:text-ink"}`}
              >
                {link.label}
              </button>
            ))}
            ...
            <div className="relative">
              <button
                ...
                className={`nav-link relative flex items-center gap-1.5 pb-1 transition-colors ${manageOpen ? "text-ink current" : "hover:text-ink"}`}
          ...
          <button type="button" className="flex h-9 w-9 items-center justify-center rounded-lg border border-line lg:hidden" onClick={() => setMobileOpen((value) => !value)} aria-expanded={mobileOpen} aria-label="Open workspace navigation">
