...
begin
  ...
  elsif new.chain_status = 'submitted' then
    new_job_status := 'submitted';
    new_mission_status := 'submitted';
  elsif new.chain_status = 'completed' then
    new_job_status := 'terminal';
    new_mission_status := 'completed';
  elsif new.chain_status = 'rejected' then
    new_job_status := 'terminal';
    new_mission_status := 'rejected';
  elsif new.chain_status = 'expired' then
    new_job_status := 'cancelled';
    new_mission_status := 'refunded';
  elsif new.chain_status = 'disputed' then
    new_job_status := 'disputed';
    new_mission_status := 'disputed';
  else
    new_job_status := new.status;
  ...
  if new_job_status is distinct from new.status then
    update public.jobs
      set status = new_job_status,
          submitted_at = case when new.chain_status = 'submitted' then coalesce(submitted_at, now()) else submitted_at end,
          terminal_at = case when new.chain_status in ('completed','rejected','expired') then coalesce(terminal_at, now()) else terminal_at end,
          funded_at = case when new.chain_status = 'funded' then coalesce(funded_at, now()) else funded_at end,
          updated_at = now()
    where id = new.id;
  else
    update public.jobs set updated_at = now() where id = new.id;
  end if;
  ...
  if user_id_value is not null then
    ...
    if new.chain_status = 'funded' then
      ...
      values(user_id_value, mission_id_value, task_id_value, coalesce(provider_wallet, ''), 'job_funded', 'New funded mission', 'A mission has been funded and is ready for the assigned provider.');
    elsif new.chain_status = 'completed' then
      insert into public.notifications(user_id, mission_id, task_id, recipient, kind, title, body)
      values(user_id_value, mission_id_value, task_id_value, coalesce(provider_wallet, ''), 'job_completed', 'Mission completed', 'The ERC-8183 job reached a completed terminal state.');
    elsif new.chain_status in ('rejected','expired') then
      insert into public.notifications(user_id, mission_id, task_id, recipient, kind, title, body)
      values(user_id_value, mission_id_value, task_id_value, coalesce(provider_wallet, ''), 'job_recovery', 'Mission entered recovery', 'The ERC-8183 job reached a rejected or expired terminal state.');
    end if;
  end if;
