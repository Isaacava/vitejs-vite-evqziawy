...
def create_erc8183_app(on_job: Callable[..., Any]) -> FastAPI:
    ...
    execution_capital_window = max(
        0,
        int(float(_env("ERC8183_EXECUTION_CAPITAL_WINDOW_SECONDS", "900") or "900")),
    )
    execution_internal_url = (
        _env("GRID_EXECUTION_INTERNAL_URL", "http://127.0.0.1:8788")
    ...
    async def process_job(job_id: int) -> None:
        ...
        try:
            ...
            if submission.get("success"):
                logger.info(
                    "Grid job %s submitted: %s",
                    job_id,
                    submission.get("txHash"),
                )
            else:
                logger.error(
                    "Grid job %s submission failed: %s",
                    job_id,
    ...
    async def negotiate(request: Request):
        ...
        if not isinstance(body, dict) or "terms" not in body:
            raise HTTPException(
                status_code=400,
                detail="Request must include terms",
            )
