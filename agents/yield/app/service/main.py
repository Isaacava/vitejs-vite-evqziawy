...
async def on_funded(job:dict[str,Any])->None:
    ...
    try:
        ...
        if status not in {"observed","evaluated","executed"}:
            raise RuntimeError("Yield agent did not produce an accepted execution status")
        if status=="executed" and not metadata.get("transaction_hash"):
            raise RuntimeError("Yield execution reported executed but no transaction hash was returned; refusing submission")
