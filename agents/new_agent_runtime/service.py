...
def _manifest(base: str):
    ...
    return {
        "spec": "agent-provider/v1", "name": DISPLAY_NAME,
        "description": os.getenv("AGENT_PROVIDER_DESCRIPTION", f"{DISPLAY_NAME} provider on BSC Testnet."),
        "version": os.getenv("AGENT_PROVIDER_VERSION", "1.0.0"), "agent": {"provider": "AgentMarket first-party"},
        "protocols": ["erc-8183", "http"], "networks": [{"chain_id": CHAIN_ID, "name": "BSC Testnet", "environment": "testnet"}],
        ...
        "execution": {"mode": "provider-watcher", "authorization": "erc8183-job-scoped", "state_changing": bool(os.getenv("AGENT_STATE_CHANGING", "false").lower() in {"1","true","yes"}), "user_approval_required": True},
...
async def preflight(request: FastAPIRequest):
    body = await request.json()
    req = Request(EXECUTION_URL + "/preflight", data=json.dumps(body).encode(), headers={"content-type":"application/json"}, method="POST")
    try:
