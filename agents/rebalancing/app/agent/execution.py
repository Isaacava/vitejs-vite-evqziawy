...
def resolve_job_execution_wallet(*, job_id: int, explicit_wallet: str | None = None) -> str:
    ...
    try:
        # ERC8183_AGENT_URL already points at the provider's /erc8183 base path.
        ...
        raise RuntimeError(
            "Unable to resolve the provider wallet needed to verify job-scoped execution authorization"
        ) from exc
    ...
    if not isinstance(authorization, dict):
        raise RuntimeError(
            "This funded ERC-8183 job has no Altana execution authorization yet. "
            "Complete the user authorization step before the agent can execute."
        )
    wallet = authorization.get("execution_wallet")
    if not _valid_address(wallet):
        raise RuntimeError(
            "Altana execution authorization is incomplete: no valid user execution wallet is recorded for this job"
        )
    return str(wallet)
...
def execute_testnet_swap(*, job_id: int, wallet_address: str | None, token_in: str, token_out: str, amount_in: str, amount_out_minimum: str, fee: int = 2500) -> dict:
    ...
    if payload.get("status") == "FAILED":
        raise RuntimeError("Altana execution reported FAILED")
    return payload
