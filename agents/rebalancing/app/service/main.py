...
def save_response(job_id: int, deliverable: str, metadata: dict[str, Any], tx_hash: str | None) -> None:
    ...
    The response endpoint is part of the ERC-8183 service contract. Keep the
    submitted deliverable available after the on-chain submit succeeds rather
    than deleting the only local record with clear_pending().
    """
    STORAGE_DIR.mkdir(parents=True, exist_ok=True)
...
def _job_matches_agent(job: dict[str, Any], has_pending: bool = False) -> bool:
    ...
    if not assigned:
        logging.warning(
            "%s funded job=%s has no provider assignment in watcher payload; refusing to execute",
            DISPLAY_NAME,
            job.get("jobId"),
        )
        return False
...
async def submit(job_id: int, deliverable: str, metadata: dict[str, Any], job: dict[str, Any]):
    ...
    if tx_hash is None and isinstance(result, dict):
        tx_hash = result.get("hash") or result.get("tx_hash")
    if tx_hash is None and isinstance(result, str):
        tx_hash = result
    tx_hash = str(tx_hash) if tx_hash else None
    try:
        save_response(job_id, deliverable, metadata, tx_hash)
    except Exception:
        # The chain is already submitted at this point. Do not raise a second
        # submission attempt because local response persistence failed.
        logging.exception("%s submitted job=%s but could not persist the response artifact", DISPLAY_NAME, job_id)
...
async def on_funded(job: dict[str, Any]) -> None:
    try:
        ...
        logging.warning("%s funded callback missing valid jobId", DISPLAY_NAME)
        return
    if not _is_funded(job):
        logging.warning(
            "%s received non-FUNDED callback job=%s status=%r; ignoring",
            DISPLAY_NAME,
            job_id,
            job.get("status"),
        )
    ...
    if not _job_matches_agent(job, has_pending=pending is not None):
        logging.info(
            "%s ignoring funded job=%s because on-chain provider does not match this agent",
            DISPLAY_NAME,
            job_id,
        )
        return
    try:
        ...
        if status == "planned" or (decision in {"move_range", "widen"} and status != "executed"):
            raise RuntimeError(
                f"Rebalancing decision '{decision or 'state-changing'}' requires successful execution before ERC-8183 submission"
            )
        ...
        if status == "executed" and not metadata.get("transaction_hash"):
            raise RuntimeError(
                "Rebalancing execution reported executed but no transaction hash was returned; refusing submission"
            )
        _runtime["last_execution"] = {
...
def proxy_get(path: str, query: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise RuntimeError("Altana execution service returned invalid capability response")
    return payload
...
def proxy_post(path: str, body: dict[str, Any]) -> dict[str, Any]:
    ...
    with urlopen(request, timeout=float(os.getenv("ALTANA_EXECUTION_TIMEOUT", "30"))) as response:
        payload = json.loads(response.read().decode("utf-8"))
    if not isinstance(payload, dict):
        raise RuntimeError("Altana execution service returned invalid response")
...
async def lifespan(_: FastAPI):
    ...
    logging.info(
        "%s watcher starting provider=%s network=%s chain_id=%s poll=%ss",
        DISPLAY_NAME,
        provider_address(),
        NETWORK,
        CHAIN_ID,
...
app = FastAPI(
    ...
    description=f"Standalone Testnet-only ERC-8183 {DISPLAY_NAME} provider",
    lifespan=lifespan,
)
...
@app.get("/health")
async def health():
    return {"status": "ok", "agent": KIND, "network": NETWORK, "chain_id": CHAIN_ID}
...
async def root():
    return {
        "status": "ok",
        "service": f"{DISPLAY_NAME} ERC-8183 provider",
        "agent_kind": KIND,
        "network": NETWORK,
        "chain_id": CHAIN_ID,
...
async def erc_health():
    return {"status": "ok", "service": DISPLAY_NAME, "network": NETWORK, "chain_id": CHAIN_ID}
...
async def status():
    return {
        ...
        "execution_service": EXECUTION_URL,
    }
...
async def runtime():
    return {
        ...
        "last_funded_job": _runtime["last_funded_job"],
...
async def preflight(request: Request):
    ...
    try:
        return proxy_post("/preflight", body)
    except Exception as exc:
...
async def negotiate(request: Request):
    ...
    return {
        "accepted": True,
        "quote_id": f"{KIND}-{int(time.time())}",
        "price": str(SERVICE_PRICE),
        "currency": payment_token() or "testnet-settlement-token",
