...
async function syncReceipt(req: VercelRequest, res: VercelResponse, auth: Awaited<ReturnType<typeof getAuthenticatedUser>>) {
  ...
  if (!success) return res.status(409).json({ error: "Blockchain transaction reverted", tx_hash: txHash, status: receipt.status });
  ...
  if (phase === "create") {
    confirmedChainStatus = "created";
  } else if (phase === "register") {
    confirmedChainStatus = "registered";
  } else if (phase === "set_budget") {
    confirmedChainStatus = "budget_set";
  } else if (phase === "fund") {
