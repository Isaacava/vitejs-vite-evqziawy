...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const token = await publicClient.readContract({ address: COMMERCE, abi: TOKEN_ABI, functionName: "paymentToken" });
    const [decimals, symbol, balance, allowance] = await Promise.all([
      ...
      publicClient.readContract({ address: token, abi: ERC20_ABI, functionName: "balanceOf", args: [clientAddress as Address] }),
      publicClient.readContract({ address: token, abi: ERC20_ABI, functionName: "allowance", args: [clientAddress as Address, COMMERCE] }),
    ]);
    const rawBudget = parseUnits(budget, decimals);
    ...
    return res.status(200).json({
      ...
      payment: { token, symbol, decimals, budget_raw: rawBudget.toString(), balance_raw: balance.toString(), allowance_raw: allowance.toString(), balance_formatted: formatUnits(balance, decimals), allowance_formatted: formatUnits(allowance, decimals) },
      expiry: new Date(Number(expiresAt) * 1000).toISOString(),
      wallet_steps: ["createJob", "registerJob using the returned jobId", "setBudget using the returned jobId", allowance < rawBudget ? "approve payment token to Commerce" : "approval already sufficient", "fund using the returned jobId"],
      transactions: {
        ...
        approve: allowance < rawBudget ? { to: token, data_builder: "Encode ERC-20 approve(Commerce, budgetRaw) in the user wallet." } : { data_builder: "No approval transaction required." },
