...
async function resolveRegistration(uri: string): Promise<RegistrationFile> {
  if (uri.startsWith("data:")) {
    ...
    const text = uri.includes(";base64") ? Buffer.from(payload, "base64").toString("utf8") : decodeURIComponent(payload);
    return JSON.parse(text) as RegistrationFile;
  }
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 7000);
  try {
    const response = await fetch(normalizeUri(uri), { signal: controller.signal, headers: { accept: "application/json,text/plain;q=0.9,*/*;q=0.8" } });
...
function strings(value: unknown): string[] {
  ...
  return value.flatMap((item) => {
    ...
    if (item && typeof item === "object") {
      ...
      return [record.name, record.id, record.capability, record.skill, record.description].filter((part): part is string => typeof part === "string");
    }
    return [];
  });
}
...
function extractEndpoints(registration: RegistrationFile): Endpoint[] {
  const raw = [...strings(registration.endpoints), ...strings(registration.services)];
  ...
  const objectEndpoints = objects.flatMap((item) => {
    ...
    const record = item as Record<string, unknown>;
    const url = [record.serviceEndpoint, record.endpoint, record.url, record.uri].find((candidate): candidate is string => typeof candidate === "string" && candidate.startsWith("http"));
    if (!url) return [];
    return [{ url, protocol: typeof record.protocol === "string" ? record.protocol : typeof record.name === "string" ? record.name : "erc8183", version: typeof record.version === "string" ? record.version : undefined, metadata: record }];
  });
  const stringEndpoints = raw.filter((value) => /^https?:\/\//i.test(value)).map((url) => ({ url, protocol: "erc8183" }));
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (process.env.NODE_ENV === "production" && !cronSecret) return res.status(503).json({ error: "CRON_SECRET must be configured before production indexing is enabled" });
  try {
