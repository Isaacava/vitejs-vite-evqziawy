...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { data: existing, error: existingError } = await supabase.from("agents").select("id,metadata,name,description,category,is_first_party").eq("agent_id", agentIdValue).maybeSingle();
    ...
    const patch = {
      ...
      last_indexed_at: now,
      metadata: { ...(existing?.metadata || {}), environment: "testnet", registry: REGISTRY, synced_at: now },
    };
    ...
    if (existing) {
      ...
      const { data, error } = await supabase.from("agents").insert({
        ...
        owner: String(onchainOwner),
        uri: String(uri),
        name: `Testnet Agent #${agentIdValue}`,
        description: "ERC-8004 agent discovered on BSC Testnet.",
        ...
        indexed_at: now,
        last_indexed_at: now,
        metadata: { environment: "testnet", registry: REGISTRY, synced_at: now },
      }).select("id,agent_id,name,chain,status,verification_status,owner,uri,category,is_first_party").single();
      if (error) throw new Error(error.message);
      rowId = data.id;
    }

    await supabase.from("agent_capabilities").upsert({ agent_id: rowId, capability: "grid_trading", source: "testnet_registration", confidence: 1, metadata: { environment: "testnet", agentId: agentIdValue }, updated_at: now }, { onConflict: "agent_id,capability,source" });
