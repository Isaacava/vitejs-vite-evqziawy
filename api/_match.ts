...
function scoreAgent(
  ...
  onchain: OnchainAgentStats | null,
) {
  ...
  if (reputationAvailable) reasons.push("ERC-8004 reputation history available onchain");
  else reasons.push("ERC-8004 reputation history not yet available");
...
async function readOnchainBatch(agents: AgentRow[]) {
  ...
  for (let start = 0; start < agents.length; start += 10) {
    ...
    const values = await Promise.all(batch.map(async (agent) => {
      try {
        return [agent.agent_id, await readAgentOnchainStats(agent.agent_id)] as const;
      } catch {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const matches = candidateAgents
      .map((agent) => ({ agent, ...scoreAgent(agent, intent, endpointByAgent.get(agent.id), reputationByAgent.get(agent.id) ?? [], onchainByAgent.get(agent.agent_id) ?? null) }))
