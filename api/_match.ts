...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const matches = candidateAgents.map((agent) => ({ agent, ...scoreAgent(agent, intent, endpointByAgent.get(agent.id), reputationByAgent.get(agent.id) ?? [], onchainByAgent.get(agent.agent_id) ?? null) })).sort((a, b) => a.hireability.canCreateJob !== b.hireability.canCreateJob ? (a.hireability.canCreateJob ? -1 : 1) : b.score - a.score).slice(0, 10);
    const bestHireableMatch = matches.find((match) => match.hireability.canCreateJob) ?? null;
    return res.status(200).json({ intent, bestMatch: matches[0] ?? null, bestHireableMatch, alternatives: matches.slice(1), network: { environment: "testnet", chain: "bsc-testnet", chain_id: 97 }, scoring: { weights: WEIGHTS, hireabilityPolicy: "Only BSC Testnet agents with a currently healthy Testnet provider endpoint are hireable.", jobHistorySource: "ERC-8183 Commerce on BSC Testnet; cached in Supabase by scheduled chain sync with live RPC fallback", reputationSource: "ERC-8004 Reputation Registry on BSC Testnet; Supabase reputation rows are fallback cache only" } });
  } catch (error) {
