...
type ChainJob = { id: bigint; client: Address; status: number; submittedAt: bigint; deliverable: `0x${string}` };
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const chainJob = await publicClient.readContract({ address: PROVIDER_ERC8183_TESTNET.commerce, abi: JOB_READ_ABI, functionName: "getJob", args: [BigInt(jobId)], ...READ_OPTIONS }) as unknown as ChainJob;
    ...
    const simulation = await publicClient.simulateContract({ address: PROVIDER_ERC8183_TESTNET.router, abi: ROUTER_ABI, functionName: "settle", args: [BigInt(jobId), "0x"], account: auth.user.wallet_address as Address });
    return res.status(200).json({
      ok: true, network: "bsc-testnet", chain_id: 97, job_id: jobId, marketplace_job_id: marketplaceJobId,
    ...
    return res.status(409).json({ error: error instanceof Error ? error.message : "Settlement is not available", network: "bsc-testnet", chain_id: 97 });
  }
}
