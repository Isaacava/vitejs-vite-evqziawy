...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!request) return res.status(200).json({ ok: true, required: false, status: "not_requested", authorization: null, erc8183: { job_id: Number(jobId), status: Number(chainJob.status), provider: chainJob.provider } });
    ...
    return res.status(200).json({ ok: true, required: true, status: String(request.status || "requested").toLowerCase(), request_id: request.id, erc8183: { job_id: Number(jobId), status: Number(chainJob.status), provider: chainJob.provider }, authorization: request.status === "authorized" ? { execution_wallet: request.user_execution_wallet || null, session_key_id: request.session_key_id || null, session_key_address: request.agent_session_key || null, session_expiry: request.session_expiry || null, capital_token: request.capital_token || executionMarket.token_in || null, capital_authorized: request.capital_authorized || request.capital_requested || null, wallet_provider: "altana", authorization_model: "scoped_session", allowed_targets: Array.isArray(capability.allowed_targets) ? capability.allowed_targets : [], allowed_selectors: Array.isArray(capability.allowed_selectors) ? capability.allowed_selectors : [] } : null });
