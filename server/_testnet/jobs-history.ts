...
import { getAuthenticatedUser, serverClient, setPrivateNoStore } from "../../src/server/authHandlers.js";
...
const BATCH_SIZE = 25; const MAX_SCAN = 2000;
const JSON_LABEL_KEYS = ["title", "mission_title", "task_title", "name", "goal", "description", "summary", "label"];

type ChainJob = {
  id: bigint; client: Address; provider: Address; evaluator: Address; description: string; budget: bigint; expiredAt: bigint; status: number;
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
        ...
        for (const key of JSON_LABEL_KEYS) { const candidate = record[key]; if (typeof candidate === "string" && candidate.trim()) return candidate.trim(); }
        const nested = record.mission;
        if (nested && typeof nested === "object" && !Array.isArray(nested)) for (const key of JSON_LABEL_KEYS) { const candidate = (nested as Record<string, unknown>)[key]; if (typeof candidate === "string" && candidate.trim()) return candidate.trim(); }
      }
      break;
    } catch { break; }
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { data: tasks } = taskIds.length ? await supabase.from("mission_tasks").select("id,mission_id,title").in("id", taskIds) : { data: [] };
    const missionIds = Array.from(new Set((tasks ?? []).map((task) => task.mission_id).filter(Boolean)));
    const { data: missions } = missionIds.length ? await supabase.from("missions").select("id,title,status,user_id,client_wallet").in("id", missionIds) : { data: [] };
    const { data: providerAgents } = providerAgentIds.length ? await supabase.from("agents").select("id,agent_id,name,owner,category,verification_status").in("id", providerAgentIds) : { data: [] };
    ...
    const jobs = userChainJobs.sort((a, b) => Number(b.id - a.id)).map((chainJob) => {
      ...
      return {
        ...chain, id: db?.id ?? null, mission_id: mission?.id ?? null, mission_title: missionTitle, mission_status: mission?.status ?? null, task_title: taskTitle,
        job_status: chain.chain_status.toLowerCase(), budget: db?.budget ?? null, agent: providerAgent ? { id: providerAgent.id, agent_id: providerAgent.agent_id, name: providerAgent.name, category: providerAgent.category ?? null, verification_status: providerAgent.verification_status ?? null } : null,
        submitted_at: submitted && chainJob.submittedAt > 0n ? new Date(Number(chainJob.submittedAt) * 1000).toISOString() : null, created_at: db?.created_at ?? null, funded_at: db?.funded_at ?? null,
        terminal_at: TERMINAL.has(chain.chain_status) ? (db?.terminal_at ?? null) : null, updated_at: db?.updated_at ?? null, marketplace_recorded: Boolean(db),
      };
    });
