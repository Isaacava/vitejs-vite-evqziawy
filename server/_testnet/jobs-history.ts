...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!Number.isFinite(latest) || latest <= 0) {
      return res.status(200).json({ ok: true, network: "bsc-testnet", chain_id: 97, source_of_truth: "erc8183_commerce", viewer_role: "client", jobs: [] });
    }
    ...
    // A wallet may also own an agent and therefore appear as a provider for other users' jobs.
    // Provider-side jobs must not be rendered as the user's own missions.
    ...
    const jobs = userChainJobs
      ...
      .map((chainJob) => {
        const chain = serializeChainJob(chainJob, "client");
