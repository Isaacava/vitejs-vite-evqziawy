...
const COMMERCE_ABI = [{
  ...
  outputs: [{ name: "job", type: "tuple", components: [
    ...
    { name: "evaluator", type: "address" }, { name: "expiredAt", type: "uint256" }, { name: "description", type: "string" },
    { name: "budget", type: "uint256" }, { name: "status", type: "uint8" }, { name: "deliverable", type: "string" }, { name: "hook", type: "address" },
  ] }],
}] as const;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { data: job, error: jobError } = await supabase.from("jobs").select("id,mission_task_id,client_wallet,status,chain_job_id,chain_status,deliverable,submitted_at,terminal_at,updated_at").eq("id", jobId).maybeSingle();
    if (jobError) throw new Error(jobError.message);
    if (!job) return res.status(404).json({ error: "Marketplace job not found" });

    const task = job.mission_task_id ? (await supabase.from("mission_tasks").select("id,mission_id").eq("id", job.mission_task_id).maybeSingle()).data : null;
    ...
    if (storedChainJobId) {
      chainJob = await publicClient.readContract({ address: COMMERCE, abi: COMMERCE_ABI, functionName: "getJob", args: [BigInt(storedChainJobId)], ...READ_OPTIONS }) as unknown as {
        id: bigint; client: Address; provider: Address; evaluator: Address; budget: bigint; status: number; expiredAt: bigint; deliverable: string; description: string;
      };
    ...
    const needsSync = Boolean(storedChainJobId) && persistedStatus !== chainStatus;
    return res.status(200).json({
      ok: true, network: "bsc-testnet", chain_id: 97,
      marketplace_job: { id: job.id, status: job.status, chain_job_id: job.chain_job_id, chain_status: persistedStatus, updated_at: job.updated_at },
      onchain_job: chainJob ? { id: chainJob.id.toString(), client: chainJob.client, provider: chainJob.provider, evaluator: chainJob.evaluator, budget: chainJob.budget.toString(), status: Number(chainJob.status), status_name: chainStatus, expired_at: new Date(Number(chainJob.expiredAt) * 1000).toISOString(), deliverable: chainJob.deliverable, description: chainJob.description } : null,
      recovery: { next_step: nextStep(chainStatus, Boolean(storedChainJobId)), requires_chain_sync: needsSync, source_of_truth: "bsc-testnet", can_resume: chainStatus !== "COMPLETED" && chainStatus !== "REJECTED" },
    });
  } catch (error) {
