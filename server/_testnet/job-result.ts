  ...
  try {
    ...
    if (archived?.verified && archived.content_base64) {
      ...
      if (String(chainJob.deliverable).toLowerCase() === computedHash.toLowerCase()) {
        ...
        return res.status(200).json({ ok: true, chain_job_id: Number(chainJob.id), chain_status: Number(chainJob.status), provider: chainJob.provider, client: chainJob.client, submitted_at: Number(chainJob.submittedAt), onchain_deliverable_hash: chainJob.deliverable, computed_deliverable_hash: computedHash, verified: true, response_bytes: bytes.length, endpoint: archived.provider_endpoint, agent_id: agent?.agent_id ?? null, agent_name: agent?.name ?? null, content, evidence_source: "agentmarket_archive", captured_at: archived.captured_at, capture_source: archived.capture_source, archive_available: true });
      }
    }

    if (!agent) return res.status(200).json({ ok: true, chain_job_id: Number(chainJob.id), chain_status: Number(chainJob.status), provider: chainJob.provider, client: chainJob.client, submitted_at: Number(chainJob.submittedAt), onchain_deliverable_hash: chainJob.deliverable, computed_deliverable_hash: null, verified: false, response_bytes: null, endpoint: archived?.provider_endpoint ?? null, agent_id: null, agent_name: null, content: null, evidence_source: "onchain_only", archive_available: Boolean(archived), verification_error: archived?.verification_error ?? "No registered AgentMarket endpoint for this provider." });
