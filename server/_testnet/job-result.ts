...
function findTransactionHash(value: unknown): Hex | null {
  ...
  for (const key of ["execution_result", "execution", "evidence", "transactions", "receipt", "response", "content", "result", "metadata"]) { const nested = findTransactionHash(record[key]); if (nested) return nested; }
  return null;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (archived?.verified && archived.content_base64) {
      const bytes = decodeBase64(archived.content_base64); const computedHash = keccak256(bytes) as Hex;
      if (String(chainJob.deliverable).toLowerCase() === computedHash.toLowerCase()) {
        const content = parseContent(bytes); await bridgeDeliverableTransactionPointer(supabase, Number(chainJob.id), content);
        return res.status(200).json({ ok: true, chain_job_id: Number(chainJob.id), chain_status: Number(chainJob.status), provider: chainJob.provider, client: chainJob.client, submitted_at: Number(chainJob.submittedAt), onchain_deliverable_hash: chainJob.deliverable, computed_deliverable_hash: computedHash, verified: true, response_bytes: bytes.length, endpoint: archived.provider_endpoint, agent_id: agent?.agent_id ?? null, agent_name: agent?.name ?? null, content, evidence_source: "agentmarket_archive", captured_at: archived.captured_at, capture_source: archived.capture_source, archive_available: true, submission_anchor: { type: "erc8183-deliverable", chain_id: 97, commitment: chainJob.deliverable } });
      }
    }

    if (!agent) return res.status(200).json({ ok: true, chain_job_id: Number(chainJob.id), chain_status: Number(chainJob.status), provider: chainJob.provider, client: chainJob.client, submitted_at: Number(chainJob.submittedAt), onchain_deliverable_hash: chainJob.deliverable, computed_deliverable_hash: null, verified: false, response_bytes: null, endpoint: archived?.provider_endpoint ?? null, agent_id: null, agent_name: null, content: null, evidence_source: "onchain_only", archive_available: Boolean(archived), verification_error: archived?.verification_error ?? "No registered AgentMarket endpoint for this provider.", submission_anchor: { type: "erc8183-deliverable", chain_id: 97, commitment: chainJob.deliverable } });

    const { data: endpoints, error: endpointError } = await supabase.from("agent_endpoints").select("endpoint_url,protocol,status,metadata").eq("agent_id", String(agent.id)).order("last_checked_at", { ascending: false }).limit(20);
    ...
    for (const endpoint of (endpoints || []) as EndpointRecord[]) {
      ...
      try {
        ...
        return res.status(200).json({ ok: true, chain_job_id: Number(chainJob.id), chain_status: Number(chainJob.status), provider: chainJob.provider, client: chainJob.client, submitted_at: Number(chainJob.submittedAt), onchain_deliverable_hash: chainJob.deliverable, computed_deliverable_hash: computedHash, verified, response_bytes: bytes.length, endpoint: result.endpoint, agent_id: agent.agent_id, agent_name: agent.name, content, evidence_source: "provider_declared_operation", operation: { action: operation.action, endpoint: operation.endpoint, method: operation.method, transport: operation.transport, name: operation.name }, archive_available: Boolean(archived), verification_error: verified ? null : `Hash mismatch: computed ${computedHash}, on-chain ${chainJob.deliverable}`, submission_anchor: { type: "erc8183-deliverable", chain_id: 97, commitment: chainJob.deliverable } });
      } catch (error) { lastError = error instanceof Error ? error.message : "Provider result operation failed"; }
    }

    return res.status(200).json({ ok: true, chain_job_id: Number(chainJob.id), chain_status: Number(chainJob.status), provider: chainJob.provider, client: chainJob.client, submitted_at: Number(chainJob.submittedAt), onchain_deliverable_hash: chainJob.deliverable, computed_deliverable_hash: null, verified: false, response_bytes: null, endpoint: archived?.provider_endpoint ?? null, agent_id: agent.agent_id, agent_name: agent.name, content: null, evidence_source: "provider_operation_pending", archive_available: Boolean(archived), verification_error: lastError, submission_anchor: { type: "erc8183-deliverable", chain_id: 97, commitment: chainJob.deliverable } });
  } catch (error) { return res.status(500).json({ error: error instanceof Error ? error.message : "Unable to verify provider result" }); }
}
