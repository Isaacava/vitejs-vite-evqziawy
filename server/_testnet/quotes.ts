...
async function requestQuote(req: VercelRequest, res: VercelResponse, user: NonNullable<Awaited<ReturnType<typeof getAuthenticatedUser>>>) {
  ...
  const requestMetadata = typeof req.body?.parameters === "object" && req.body.parameters !== null ? req.body.parameters as Record<string, unknown> : {};
  ...
  if (!/^0x[a-fA-F0-9]{40}$/.test(user.user.wallet_address)) return res.status(400).json({ error: "Authenticated wallet address is invalid" });
  ...
  const quoteHash = keccak256(stringToHex(canonicalQuotePayload({ quoteId, agentId: agent.id, requesterWallet, goal, price, currency, expiresAt, requestMetadata })));
...
async function acceptQuote(req: VercelRequest, res: VercelResponse, user: NonNullable<Awaited<ReturnType<typeof getAuthenticatedUser>>>) {
  ...
  const supabase = serverClient();
  const { data: quote, error } = await supabase.from("marketplace_quotes").select("quote_id,requester_wallet,status,expires_at,quote_hash,chain_id,environment").eq("quote_id", quoteId).maybeSingle();
  ...
  return res.status(200).json({ ok: true, network: "bsc-testnet", chain_id: TESTNET_CHAIN_ID, environment: TESTNET_ENVIRONMENT, quote: { ...accepted, price_raw: String(accepted.price), price_formatted: formatUnits(BigInt(accepted.price), payment.decimals) }, payment: { symbol: payment.symbol, decimals: payment.decimals }, next: "Use quote_id with /api/testnet/prepare-quote to prepare the accepted Testnet job." });
}
