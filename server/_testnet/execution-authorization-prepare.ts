...
async function capability(agent: Record<string, unknown>): Promise<{ descriptor: Record<string, unknown>; source_url: string }> {
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (new TextEncoder().encode(raw).byteLength > MAX_CAPABILITY_BYTES) throw new Error("response too large");
      const value = object(raw ? JSON.parse(raw) : null);
      if (value.network !== "bsc-testnet" || Number(value.chainId) !== 97) throw new Error("not BSC Testnet");
      if (value.execution !== "altana-scoped-session" || value.wallet_provider !== "altana" || value.authorization_model !== "scoped_session") throw new Error("provider does not advertise Altana scoped sessions");
      if (value.private_key_exposed !== false || !address(value.session_key_address) || !hex(value.session_key_public_key)) throw new Error("invalid public execution capability");
      if (!Array.isArray(value.allowed_targets) || !Array.isArray(value.allowed_selectors) || !value.allowed_targets.length || !value.allowed_selectors.length) throw new Error("execution allowlists are missing");
      const market = object(value.execution_market);
      if (!address(market.token_in)) throw new Error("execution token is missing");
      return { descriptor: value, source_url: candidate };
    } catch (error) { failures.push(`${candidate}: ${error instanceof Error ? error.message : "capability failed"}`); }
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const duration = Number(req.body?.duration_seconds ?? 24 * 60 * 60);
    const amount = Number(req.body?.capital_requested ?? 1);
    if (!jobId || !/^\d+$/.test(chainJobId) || amount !== 1 || !Number.isInteger(duration) || duration < 300 || duration > 7 * 24 * 60 * 60) return res.status(400).json({ error: "job_id, chain_job_id, exactly 1 U, and a valid duration are required" });
    ...
    const token = market.token_in as Address;
    ...
    const { data: request, error: insertError } = await supabase.from("execution_capital_requests").insert({
      ...
      evidence: {
        ...
        execution_capability: { ...cap.descriptor, source_url: cap.source_url, fetched_at: fetchedAt, independently_authorized: false },
      },
    }).select("*").single();
    if (insertError) throw new Error(insertError.message);
    return res.status(201).json({ ok: true, required: true, created: true, request, chain_job_id: Number(chainJobId) });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to prepare execution authorization";
