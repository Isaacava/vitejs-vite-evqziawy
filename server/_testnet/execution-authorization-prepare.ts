...
async function capability(agent: Record<string, unknown>): Promise<{ descriptor: Record<string, unknown>; source_url: string } | null> {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (jobError) throw new Error(jobError.message); if (!job || !job.mission_task_id) return res.status(404).json({ error: "Marketplace job not found" });
    ...
    if (taskError) throw new Error(taskError.message); if (!task?.mission_id || !task.agent_id) return res.status(409).json({ error: "Job does not identify a provider agent" });
    const { data: mission, error: missionError } = await supabase.from("missions").select("id,user_id").eq("id", task.mission_id).maybeSingle();
    if (missionError) throw new Error(missionError.message); if (!mission || mission.user_id !== auth.user.id) return res.status(403).json({ error: "You do not own this mission" });
    ...
    if (agentError) throw new Error(agentError.message); if (!agent) return res.status(404).json({ error: "Provider agent not found" });
    ...
    const { data: request, error: insertError } = await supabase.from("execution_capital_requests").insert({ job_id: job.id, requester_wallet: auth.user.wallet_address, user_execution_wallet: null, agent_session_key: cap.descriptor.session_key_address, capital_requested: "1", capital_token: market.token_in, purpose, duration_seconds: duration, wallet_provider: "altana", authorization_model: "scoped_session", status: "requested", evidence: { source: "agentmarket_execution_authorization_prepare", chain_id: 97, chain_job_id: Number(chainJobId), provider_agent_id: agent.agent_id, execution_capability: { ...cap.descriptor, source_url: cap.source_url, fetched_at: fetchedAt, independently_authorized: false } } }).select("*").single();
