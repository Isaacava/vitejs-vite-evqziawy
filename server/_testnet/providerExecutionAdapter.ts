...
export function assertProviderExecutionCapability(capability: Record<string, unknown>) {
  if (capability.execution !== "altana-scoped-session" || capability.wallet_provider !== "altana" || capability.authorization_model !== "scoped_session") {
    throw new Error("This Testnet execution adapter requires a verified Altana scoped-session execution capability");
  }
  if (capability.network !== "bsc-testnet" || Number(capability.chain_id ?? capability.chainId) !== 97) {
    throw new Error("Provider execution capability is not for BSC Testnet");
  }
}
...
export async function runProviderPreflight(request: Record<string, unknown>, input: ProviderPreflightInput) {
